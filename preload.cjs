...
contextBridge.exposeInMainWorld('api', {
  ...
  cookies: {
    export: (opt) => ipcRenderer.invoke('cookies:export', opt),
    import: (arr) => ipcRenderer.invoke('cookies:import', arr),
  },
  changePassword: (pwd) => ipcRenderer.invoke('auth:change-password', pwd),
  saveCookies: (list) => ipcRenderer.invoke('auth:save-cookies', list),
  loadCookies: () => ipcRenderer.invoke('auth:load-cookies'),
});
contextBridge.exposeInMainWorld('auth', {
  ...
  accountInfo: () => ipcRenderer.invoke('auth:account-info'),
  changePassword: (pwd) => ipcRenderer.invoke('auth:change-password', pwd),
  syncNow: (localState) => ipcRenderer.invoke('auth:sync-now', localState)
});
